...
NB_MODULE(_core, m)
{
    ...
    nb::class_<StateMachine, PyStateMachine>(
        ...
        .def_static("advance_all", nb::overload_cast<std::vector<nb::ref<Walker>>&, const std::string&>(&StateMachine::advance_all), "Advance multiple walkers with a token")
        .def_static("advance_all", nb::overload_cast<std::vector<nb::ref<Walker>>&, const std::string&, const tsl::htrie_set<char>&>(&StateMachine::advance_all), "Advance multiple walkers with a token, validating against vocabulary")
    ...
    nb::class_<Walker, PyWalker>(
        ...
        nb::intrusive_ptr<Walker>([](Walker *o, PyObject *po) noexcept
                                  { o->set_self_py(po); }))
        .def(nb::init<nb::ref<StateMachine>, std::optional<StateMachine::State>>(),
             "state_machine"_a, "current_state"_a = nb::none())

        // Properties
        .def_prop_rw("state_machine", [](Walker &w)
                     { return w.state_machine_; }, [](Walker &w, StateMachine *sm)
                     { w.state_machine_ = sm; })
    ...
    nb::class_<AcceptedState, Walker>(m, "AcceptedState")
        ...
        .def_prop_rw("accepted_walker", [](AcceptedState &w)
                     { return w.accepted_walker_; }, [](AcceptedState &w, nb::ref<Walker> walker)
